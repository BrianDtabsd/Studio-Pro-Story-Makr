...
import { AIAnalyzedScript, CharacterVoicePreset, SynthesizedChunk, PresetVoiceKey, StoryIdea } from '../../types.ts';
...
interface Props {
  ...
  onCharacterVoicePresetsChange?: (presets: Record<string, CharacterVoicePreset>) => void;
...
export const TextToSpeech: React.FC<Props> = ({
  ...
  defaultVoiceKey, onDefaultVoiceKeyChange, characterVoicePresets, onCharacterVoicePresetsChange,
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // Auto-assign voices from AI analysis
      if (result.characterVoices && onCharacterVoicePresetsChange) {
        const newPresets = { ...characterVoicePresets };
        let updated = false;
        Object.entries(result.characterVoices).forEach(([charName, voiceKey]) => {
          if (!newPresets[charName] || newPresets[charName].assignedVoiceKey !== voiceKey) {
            newPresets[charName] = { assignedVoiceKey: voiceKey as PresetVoiceKey };
            updated = true;
          }
        });
        if (updated) {
          onCharacterVoicePresetsChange(newPresets);
        }
      }
    } catch (e: any) {
  ...
  const synthesizeEpisode = async (episodeId: string, script: string, analyzed: AIAnalyzedScript) => {
    ...
    for (const scene of analyzed.scenes) {
      ...
      if (!scene.dialogue || scene.dialogue.length === 0) {
        // If no dialogue, we still create a chunk but with empty audio, or we can just skip it.
        // Let's skip audio generation but not break the loop.
        continue;
      }
      ...
      try {
        ...
        console.error(`Failed to synthesize audio for scene ${scene.sceneNumber}:`, err);
          ...
          {!currentAnalyzedScript ? (
            ...
            <div className="space-y-10">
              <div className="neu-pressed p-6 rounded-2xl space-y-8">
                ...
                {currentAnalyzedScript.allCharacters && currentAnalyzedScript.allCharacters.length > 0 && (
                  <div>
                    <label className="text-sm font-bold text-neu-text-dark mb-3 block">Character Voices</label>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {currentAnalyzedScript.allCharacters.map(charName => (
                        <div key={charName} className="flex items-center justify-between bg-neu-bg p-3 rounded-xl border border-neu-border">
                          <span className="text-sm font-bold text-neu-text-dark truncate w-1/3">{charName}</span>
                          <select 
                            className="w-2/3 neu-pressed text-neu-text-dark p-2 rounded-lg text-xs focus:outline-none focus:ring-0" 
                            value={characterVoicePresets[charName]?.assignedVoiceKey || defaultVoiceKey} 
                            onChange={e => {
                              if (onCharacterVoicePresetsChange) {
                                onCharacterVoicePresetsChange({
                                  ...characterVoicePresets,
                                  [charName]: { assignedVoiceKey: e.target.value as PresetVoiceKey }
                                });
                              }
                            }}
                          >
                        ...
                        {chunk ? (
                            <div className="flex gap-2">
                                 <audio 
                                   ...
                                   src={chunk.audioDataUrl || undefined} 
                                   className="hidden" 
                                 />
                                 <button onClick={() => {
